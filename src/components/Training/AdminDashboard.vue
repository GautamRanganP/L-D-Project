...
async function loadOwners() {
  try {
    const res = await api.get(`${process.env.BACKEND_BASE_URL}/api/trainings/owners`) // no params => all trainings
...
async function fetchTrainings() {
  ...
  try {
    ...
    const res = await api.get(`${process.env.BACKEND_BASE_URL}/api/trainings`, { params })
...
function downloadReport() {
  const url = `${process.env.BACKEND_BASE_URL}/api/report-excel`
